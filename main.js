/**
 * 🏝️ CHATTYCHOP ISLAND
 * A Dynamic Island for Windows — built by G. Badrinath (Chattychop)
 * Student developer, entrepreneur & YouTuber, Hyderabad, India
 *
 * YouTube: youtube.com/@chattychop
 * Instagram: instagram.com/chatt.yx
 * GitHub: github.com/chattychop
 *
 * If you're reading this because you found these files somewhere else -
 * hey, that's cool, just wanted to say hi. Build something of your own too. 🚀
 */
...
// Bump this number whenever pushing a real update to GitHub - that's
// the entire update mechanism. Every install checks this same number
// against what's live in the repo on startup (see checkForUpdates()
// near the bottom) and updates itself automatically if it's behind.
const ISLAND_VERSION = 1;
  ...
  <div id="island">
    ...
    <div id="expanded-content">
      ...
      <div class="panel" id="panel-plan">
        ...
        <p class="section-title">Your Device ID</p>
        <div style="font-size:11px; color:rgba(255,255,255,0.5); margin-bottom:14px;">Chattychop needs this exact ID (paste it into admin's Island Users tab) before a code will work for you.</div>
        <div class="add-row" style="margin-bottom:14px;">
          <input id="plan-device-id" readonly style="cursor:text;">
...
function copyDeviceId() {
  const input = document.getElementById('plan-device-id');
  input.select();
  sendAction('set-clipboard', { text: input.value });
}
...
async function loadPlanStatus() {
  ...
  document.getElementById('plan-device-id').value = status.deviceId || '';
...
// async function declarations, unlike plain function declarations,
// don't get automatically hoisted to a global/window binding when
// declared inside a block (the outer try{} this script runs in) - so
// every async function referenced from an onclick="" attribute needs
// an explicit window assignment here, or it throws "not defined" the
// moment someone clicks the button. Plain (non-async) functions used
// in onclick don't need this - they get hoisted globally on their own.
window.signInWithGoogle = signInWithGoogle;
window.signOutOfGoogle = signOutOfGoogle;
window.askIslandPlan = askIslandPlan;
window.confirmOrganizeDownloads = confirmOrganizeDownloads;
window.previewOrganizeDownloads = previewOrganizeDownloads;
window.redeemPlanCode = redeemPlanCode;
window.runLagHunter = runLagHunter;
window.runSemanticSearch = runSemanticSearch;
window.screenshotAsk = screenshotAsk;
...
<div id="status-text" style="font-size:15px;">Sign in to Island</div>
<div id="g_id_onload"
...
<script>
  ...
  // Google calls this directly with the signed token once someone
  // signs in on the button above - no redirect/login_uri involved at
  // all, so no HTTPS requirement either. We just hand the token to our
  // own local server ourselves, from here.
  function handleCredentialResponse(response) {
    document.getElementById('status-text').textContent = 'Signing in...';
    fetch('/oauth-callback', {
      ...
      .then(function (result) {
        if (result && result.ok) {
          document.getElementById('status-text').textContent = 'Signed in as ' + result.email + ' — you can close this tab.';
          document.querySelector('.g_id_signin').style.display = 'none';
        } else {
          document.getElementById('status-text').textContent = (result && result.msg) || 'Sign-in failed - try again.';
        }
      })
      .catch(function (e) {
        document.getElementById('status-text').textContent = 'Something went wrong reaching Island.';
        debugLines.push('Fetch error: ' + e.message);
        showDebug();
      });
  }
</script>
...
async function handleOAuthCallback(req, res) {
  ...
  req.on("data", (chunk) => (body += chunk));
  req.on("end", async () => {
    const respond = (ok, msgOrEmail) => {
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify(ok ? { ok: true, email: msgOrEmail } : { ok: false, msg: msgOrEmail }));
    };
    let idToken;
    try {
      idToken = JSON.parse(body).credential;
    } catch (e) {
      idToken = null;
    }
    if (!idToken) {
      respond(false, "No sign-in info came through — try the button again.");
      return;
    }
    try {
      ...
      if (!verifyRes.ok || !validIssuer || claims.aud !== GOOGLE_CLIENT_ID) {
        respond(false, "That sign-in didn't check out. Go back to Island and try again.");
        return;
      }
      const email = (claims.email || "").toLowerCase();
      if (!email) {
        respond(false, "Couldn't read your Google account email. Try again.");
        return;
      }
      ...
      respond(false, "Sign-in error: " + e.message);
    }
  });
}
...
async function handleAction(action, payload) {
  switch (action) {
    ...
    case "set-clipboard": {
      const { clipboard } = require("electron");
      clipboard.writeText((payload && payload.text) || "");
      return { ok: true };
    }
    case "smart-rename": {
...
// Gives the underlying model (which has no idea it's running inside
// Island otherwise, and will happily claim to be "made by Meta" etc.
// since that's just whichever raw model the admin's key points at) a
// real identity, a length limit appropriate for a small popup box (not
// a full chat window), and enough context to actually answer questions
// about the app and about Chattychop instead of guessing.
const ISLAND_SYSTEM_PROMPT = `You are the built-in AI inside Chattychop Island, a Windows desktop overlay app. If asked who made you or what powers you, just say you're Island's assistant, built by Chattychop - don't volunteer the underlying AI provider or model name.

Keep every reply short: 1-3 sentences, plain text, no headers or bullet lists unless the question truly needs a list. This is a small quick-answer popup, not a full chat window.

About Island (the app you're inside): a floating pill that lives at the top of the screen. Features: Chat (talk to an AI), Tasks (to-do list), Tools (quick actions - lock PC, mute, screenshot, organize downloads, find files by description), Rules (custom hotkeys and typed-phrase automations), Clip (clipboard history), Plan (Free / Pro Rs100 per month / Premium Rs299 per month, redeem codes right here), and Sign in with Google (keeps each person's chats, tasks and notes private to their own account only). It can also launch automatically when Windows starts, toggleable in Settings.

Island was built by G. Badrinath, who goes by Chattychop - a student developer, entrepreneur and YouTuber based in India. Other projects: ChattyCHOP Inbox (an AI inbox web app), ExamIntel Pro (a study tool for Indian competitive exams), JARVIS CAM (an AR camera app), CivicX (a civic infrastructure platform), and Rot to Root (a cold-chain horticulture startup).
Find Chattychop: YouTube youtube.com/@chattychop, Instagram instagram.com/chatt.yx, GitHub github.com/chattychop.`;
    ...
    if (key.startsWith("gsk_")) {
      const r = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 200,
          messages: [{ role: "system", content: ISLAND_SYSTEM_PROMPT }, { role: "user", content: prompt }],
        }),
      });
    ...
    if (key.startsWith("AIza")) {
      const r = await fetch(
        "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=" + key,
        {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            systemInstruction: { parts: [{ text: ISLAND_SYSTEM_PROMPT }] },
...
// Checks GitHub for a newer main.js than the one currently running,
// and if found, safely replaces itself and restarts - the entire
// mechanism behind "push an update, everyone gets it automatically."
//
// Safety checks, in order, ALL must pass before anything gets touched:
//   1. Remote copy must actually be bigger than a trivial/empty response
//   2. Remote copy must parse as syntactically valid JavaScript (via
//      Node's own vm.Script, which parses without ever executing it)
//   3. Remote's own embedded ISLAND_VERSION must be strictly HIGHER
//      than this copy's - a mistake or rollback in the repo can never
//      silently downgrade anyone
// If the internet's unavailable, GitHub's unreachable, or anything at
// all looks off, this fails silently and just keeps running the
// current version - it should never be able to block the app from
// opening.
async function checkForUpdates() {
  try {
    const res = await fetch("https://raw.githubusercontent.com/chattychop/chattychop-island/main/main.js");
    if (!res.ok) return;
    const remoteSrc = await res.text();
    if (remoteSrc.length < 1000) return;

    const versionMatch = remoteSrc.match(/const ISLAND_VERSION\s*=\s*(\d+)/);
    const remoteVersion = versionMatch ? parseInt(versionMatch[1], 10) : 0;
    if (!(remoteVersion > ISLAND_VERSION)) return;

    try {
      new (require("vm").Script)(remoteSrc);
    } catch (e) {
      return; // doesn't even parse as valid JS - treat as a bad/corrupt download
    }

    fs.writeFileSync(__filename, remoteSrc);

    // If package.json also changed, grab that too and force a clean
    // reinstall on the next launch by clearing node_modules - RUN_ISLAND.bat
    // already knows to run npm install whenever that folder is missing.
    try {
      const pkgRes = await fetch("https://raw.githubusercontent.com/chattychop/chattychop-island/main/package.json");
      if (pkgRes.ok) {
        const remotePkgText = await pkgRes.text();
        const remotePkg = JSON.parse(remotePkgText); // throws if malformed - caught below
        const localPkgPath = path.join(__dirname, "package.json");
        const localPkgText = fs.existsSync(localPkgPath) ? fs.readFileSync(localPkgPath, "utf-8") : "";
        if (remotePkgText.trim() !== localPkgText.trim()) {
          fs.writeFileSync(localPkgPath, remotePkgText);
      ...
      // package.json fetch/parse failed - main.js update still stands,
      // just skip touching dependencies this round.
    }

    // Hand off to RUN_ISLAND.bat for the actual relaunch (it handles
    // npm install if node_modules just got cleared above), then close
    // this instance cleanly.
    const batPath = path.join(__dirname, "RUN_ISLAND.bat");
    ...
    // Offline, GitHub unreachable, whatever - just keep running as-is.
  }
}
