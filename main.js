...
// Island's own Google Sign-In - reuses the SAME credentials.json
// JARVIS.py already expects one folder up (same Google Cloud project),
// just requesting a much smaller scope (just knowing who's signed in),
// so there's only ever one set of Google credentials to set up, not two.
const GOOGLE_CREDS_PATH = path.join(JARVIS_DIR, "credentials.json");
...
function startBridgeServer() {
  const server = http.createServer((req, res) => {
    // The one deliberate exception to the token gate: Google's own
    // browser redirect lands here after sign-in, and it obviously can't
    // present our internal X-Island-Token header. Scoped tight - only
    // GET requests to this exact path, and the only thing it can do is
    // try to redeem a `code` query param with Google's token endpoint.
    // A forged/garbage code just gets rejected by Google; there's no
    // action-execution surface here for an attacker to reach.
    const urlObj = new URL(req.url, "http://127.0.0.1:" + BRIDGE_PORT);
    if (req.method === "GET" && urlObj.pathname === "/oauth-callback") {
      handleOAuthCallback(urlObj, res);
      return;
    }
...
async function handleOAuthCallback(urlObj, res) {
  ...
  if (!creds) {
    respond("Sign-in not set up yet", "Island needs a credentials.json next to JARVIS.py first.");
    return;
  }
  try {
    ...
    if (!tokenData.access_token) {
      respond("Sign-in failed", "Go back to Island and hit Sign in again.");
      return;
    }
    ...
    data.accounts[email].name = userData.name || email.split("@")[0];
    if (tokenData.refresh_token) data.accounts[email].googleRefreshToken = tokenData.refresh_token;
...
(Get-Process -Id $pid).ProcessName`;
    ...
    case "google-signin": {
      const creds = loadGoogleCreds();
      if (!creds) return { ok: false, msg: "Sign-in isn't ready yet — give it a moment and try again." };
      const authUrl =
